...
def restart_training(model, optimizer, epoch, metrics):
    filepath = "/scratch/eecs448w24_class_root/eecs448w24_class/shared_data/brainWiz/checkpoints/eeg_e_t/"
    filename = filepath + os.listdir(filepath)[-1]
    checkpoint = torch.load(filename)
    model.load_state_dict(checkpoint['state_dict'])
    optimizer.load_state_dict(checkpoint['optimizer'])
    epoch = checkpoint['epoch']
    metrics = checkpoint['stats']
...
def train(model, optimizer, criterion, trainDat, trainLabels, valDat, valLabels):
    ...
    # make_minibatch_training_plot(trainStats)
...
def main(retrain):
    ...
    fpath = '/scratch/eecs448w24_class_root/eecs448w24_class/shared_data/brainWiz/EEG_data/chunks/'
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    labels = torch.load(fpath + 'mapping.pt').to(device)
    ...
    totBatches = labels.shape[0] #So after loadBatches its 1759 batches.         should be mapping.shape[0] * batches_per_chunk or something
    ...
    metrics = [] #list of metrics from each epoch- across chunks
    
    dataset = DDPDataset(fpath+'eeg_data.h5',('meg_data','labels'))
    dataloader = DataLoader(dataset, batch_size=128, shuffle=True,pin_memory=True,pin_memory_device=device)
    
    eegdata = torch.load(fpath + 'all_eeg' + ".pt",map_location=device)  #load all for now
    tr_eeg  = eegdata[tr_idx, :, :] #makes index wrt current chunk
    tr_labels  = labels[tr_idx, :] 
    val_eeg = eegdata[val_idx, :, :] #makes index wrt current chunk 
    val_labels = labels[val_idx, :]
    
    if retrain:
        model, optimizer, epoch, metrics = restart_training(model, optimizer, epoch, metrics)
    else:
    ...
    while curr_count_to_patience < patience:
        ...
        metrics.append(chunk_metrics) #TODO GLOBAL STATS SHOULD BE AVERAGE OF CHUNK STATS WHEN WE HAVE >1 CHUNK 
        save_checkpoint(model, optimizer, epoch, config("EEG-Encoder.checkpoint"), metrics)
        curr_count_to_patience, global_min_loss = early_stopping(
            metrics, curr_count_to_patience, global_min_loss
        )
    ...
    test_labels  = labels[test_idx, :, :] #Any necessary data transformations (if any)... 
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python train_eeg.py <if_retrain: bool>")
        retrain = False
    
    # script, file_path, retrain = sys.argv
    # print(f"Script: {script}, File Path: {file_path}, If retrain: {retrain}")
    main(retrain)
    
#EXTRAS

#model = EEGDecoder(63, 63, 3)
